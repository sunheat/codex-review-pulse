...
and are included in the policy digest. The host agent converts explicit values
from the user's initial prompt into those structured fields before rendering
the handoff; the program does not fuzzy-parse arbitrary prose. The standalone
host receives these values when it creates a successor and must verify them in
task readback; the repository does not maintain a model allowlist because
availability is a host capability.
