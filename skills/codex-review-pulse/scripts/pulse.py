...
def confirm_task_retirement(
    ...
    *,
    ...
    if (
        ...
        # A prior non-deletion pause ended the active wake, but exact
        # reconciliation has now restored the handoff's irreversible boundary.
        # Keep the retirement-specific latch until successor recovery consumes
        # it, while restoring ownership for that handoff-only recovery.
        state["active_wake_id"] = wake_id
    result = _decision(
        "RETIREMENT_CONFIRMED",
